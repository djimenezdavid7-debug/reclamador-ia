...
def main(page: ft.Page):
    ...
    try:
        ...
        print("DEBUG: Upload dir created")
        ...
        def archivo_seleccionado(e):
            print("DEBUG: Archivo seleccionado")
            # Paso 1: El usuario elige foto.
            ...
            # --- FIX: SUBIDA AL SERVIDOR ---
            # Necesitamos obtener una URL de subida del propio Flet
            upload_url = page.get_upload_url(file.name, 600)
            print(f"DEBUG: Generated upload url: {upload_url}")

            # --- FIX: SANITIZADOR DE URLS (Conflicto de Puertos en Render) ---
            # El problema: Flet genera la URL con el puerto interno (ej: :10000)
            # La solución: Quitamos el puerto y forzamos HTTPS
            
            u = urlparse(upload_url)
            if "onrender" in upload_url or u.scheme == "http":
                # Reconstruimos la URL:
                # esquema=https, netloc=hostname(sin puerto), el resto igual
                # upload_url = urlunparse(("https", u.hostname, u.path, u.params, u.query, u.fragment))
                # print(f"URL Sanitizada: {upload_url}") # Debug para ver si funcionó
                pass
            # -------------------------------------------------------------
    ...
    # 4. Zona de Venta (El Muro de Pago) - CORREGIDO
        ...
        print("DEBUG: Elements added to page")
...
ft.app(target=main, view=ft.AppView.WEB_BROWSER, upload_dir="uploads", port=int(os.getenv("PORT", 8080)), host="127.0.0.1")
